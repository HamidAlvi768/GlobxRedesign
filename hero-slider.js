...
const heroSlides = [
  {
    ...
    subtext:
      "GlobX will get you the missing components to keep your production running no matter how difficult the situation. As supply chain experts, we master even the biggest crises with ease and bring them to a smooth production flow.",
  },
  {
    title: "Global Sourcing Experts Worldwide",
    subtext:
      "Our worldwide network ensures you get the right parts at the right time, minimizing downtime and maximizing efficiency for your business. With a dedicated team and advanced logistics, we help you overcome any supply chain challenge.",
  },
  {
    title: "Trusted by Industry Leaders",
    subtext:
      "Join top manufacturers who rely on GlobX for reliable and fast supply chain solutions tailored to their needs. Our commitment to excellence ensures you always have the support you need to succeed.",
  },
];

const heroSlideContent = document.querySelector(".gx-hero-slide-content");
const heroDots = document.querySelectorAll(
  ".gx-hero-left .gx-sidebar-pagination .gx-dot"
);
...
function startHeroAutoSlide() {
  ...
  heroAutoSlideTimer = setInterval(() => {
    if (!heroIsTransitioning) {
      // Only auto-slide if not manually transitioning
...
function handleHeroTouchEnd(e) {
  ...
  if (
    ...
    Math.abs(deltaX) > heroDragThreshold
  ) {
    if (deltaX > 0) {
      // Swipe right - go to previous slide
      const prevIdx =
        currentHeroIdx === 0 ? heroSlides.length - 1 : currentHeroIdx - 1;
...
if (heroSlideContent) {
  // Touch events
  heroSlideContent.addEventListener("touchstart", handleHeroTouchStart, {
    passive: false,
  });
  heroSlideContent.addEventListener("touchmove", handleHeroTouchMove, {
    passive: false,
  });
...
heroDots.forEach((dot, idx) => {
  dot.addEventListener("click", () => {
    if (!heroIsTransitioning) {
      // Prevent dot clicks during transition
      setHeroSlide(idx);
    }
  });
});
...
const sidebarSlides = [
  ...
  [
    {
      icon: `<svg width="36" height="36" viewBox="0 0 36 36" fill="none"><rect x="8" y="16" width="20" height="4" rx="2" stroke="white" stroke-width="2"/><path d="M8 18H4M32 18h4" stroke="white" stroke-width="2"/></svg>`,
      label: "diodes",
    },
    {
      icon: `<svg width="36" height="36" viewBox="0 0 36 36" fill="none"><rect x="10" y="10" width="16" height="16" rx="3" stroke="white" stroke-width="2"/><rect x="16" y="16" width="4" height="4" rx="1" fill="white"/></svg>`,
      label: "mpu/mcu",
    },
    {
      icon: `<svg width="36" height="36" viewBox="0 0 36 36" fill="none"><rect x="8" y="12" width="20" height="12" rx="3" stroke="white" stroke-width="2"/><rect x="12" y="16" width="4" height="4" rx="1" fill="white"/><rect x="20" y="16" width="4" height="4" rx="1" fill="white"/></svg>`,
      label: "powircs",
    },
    {
      icon: `<svg width="36" height="36" viewBox="0 0 36 36" fill="none"><rect x="8" y="16" width="20" height="4" rx="2" stroke="white" stroke-width="2"/><path d="M8 18H4M32 18h4" stroke="white" stroke-width="2"/></svg>`,
...
const sidebarDots = document.querySelectorAll(
  ".gx-sidebar-card .gx-sidebar-pagination .gx-dot"
);
...
function handleSidebarTouchEnd(e) {
  ...
  if (
    Math.abs(deltaX) > Math.abs(deltaY) &&
    Math.abs(deltaX) > sidebarDragThreshold
  ) {
    if (deltaX > 0) {
      // Swipe right - go to previous slide
      const prevIdx =
...
if (sidebarSlideContent) {
  // Touch events
  sidebarSlideContent.addEventListener("touchstart", handleSidebarTouchStart, {
    passive: false,
  });
  sidebarSlideContent.addEventListener("touchmove", handleSidebarTouchMove, {
    passive: false,
  });
